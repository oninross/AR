...
ARjs.Anchor = function (arSession, markerParameters) {
    ...
    if (markerParameters.markersAreaEnabled === false) {
        ...
        if (
            location.hash
                .substring(1)
                .startsWith("markers-page-resolution=") === true
        ) {
            ...
            ARjs.MarkersAreaUtils.storeMarkersAreaFileFromResolution(
        ...
        if (localStorage.getItem("ARjsMultiMarkerFile") === null) {
            ARjs.MarkersAreaUtils.storeDefaultMultiMarkerFile(
                arContext.parameters.trackingBackend
            );
        }
        ...
        if (markerParameters.changeMatrixMode === "modelViewMatrix") {
            ...
            markerParameters.changeMatrixMode === "cameraTransformMatrix"
        ) {
            var parent3D = camera;
        } else console.assert(false);
        // build a multiMarkerControls
        var multiMarkerControls = ARjs.MarkersAreaControls.fromJSON(
            arContext,
            parent3D,
            controlledObject,
            multiMarkerFile
